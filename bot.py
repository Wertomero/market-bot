...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS users (
        user_id BIGINT PRIMARY KEY, username TEXT, shop_name TEXT, seller_game_email TEXT)""")
...
def set_shop(uid, shop_name, email):
    conn = get_conn()
    c = conn.cursor()
    c.execute("UPDATE users SET shop_name=%s, seller_game_email=%s WHERE user_id=%s", (shop_name, email, uid))
    conn.close()
...
def get_shop(uid):
    conn = get_conn()
    c = conn.cursor(cursor_factory=RealDictCursor)
    c.execute("SELECT shop_name, seller_game_email FROM users WHERE user_id=%s", (uid,))
    r = c.fetchone()
    conn.close()
...
async def view_cart(cb: CallbackQuery):
    ...
    for i in items:
        ...
        kb.append([
            InlineKeyboardButton(text=f"✏️ {i['name']}", callback_data=f"editcart_{i['id']}")])
    text += f"\n💰 <b>Итого: {total}</b>"
...
async def order_detail(cb: CallbackQuery):
    ...
    text += f"Статус: {emoji.get(order['status'], '?')} {order['status']}\n"
    text += f"💰 Сумма: {order['total_amount']}\n"
    text += f"📧 Почта: {order['buyer_game_email']}\n"
...
async def shop_name(msg: Message, state: FSMContext):
    ...
    await msg.answer("📧 Введите вашу игровую почту (куда покупатели будут переводить валюту):")
...
async def seller_stats(cb: CallbackQuery):
    ...
    text += f"💰 Заработано всего: <b>{stats['total_earned']}</b>\n"
