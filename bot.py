...
def get_task_responses(task_id):
    conn = get_conn()
    c = conn.cursor(cursor_factory=RealDictCursor)
    c.execute("SELECT tr.*, u.username FROM task_responses tr JOIN users u ON tr.buyer_id = u.user_id WHERE tr.task_id=%s AND tr.status='pending' ORDER BY tr.created_at DESC", (task_id,))
    return c.fetchall()
...
async def task_detail(cb: CallbackQuery):
    ...
    kb.append([InlineKeyboardButton(text="🔙 К заданиям", callback_data="all_tasks")])
    await cb.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def task_responses(cb: CallbackQuery):
    ...
    if not responses:
        await cb.message.edit_text("👀 Пока нет откликов.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def accept_response(cb: CallbackQuery, bot: Bot):
    ...
    await cb.message.edit_text("✅ Исполнитель выбран! Задание в работе.")
    responses = get_task_responses(tid)
    for r in responses:
        if r['id'] == rid:
            try:
...
async def my_tasks(cb: CallbackQuery):
    ...
    for t in tasks:
        ...
        text += f"{status_emoji} #{t['id']} — {t['description'][:50]}...\n"
        kb.append([InlineKeyboardButton(text=f"📋 Задание #{t['id']}", callback_data=f"task_{t['id']}")])
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data="task_board")])
    await cb.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
