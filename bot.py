...
def update_cart(uid, pid, qty):
    conn = get_conn()
    if qty <= 0: c = conn.cursor(); c.execute("DELETE FROM cart WHERE user_id=%s AND product_id=%s", (uid, pid))
    else: c = conn.cursor(); c.execute("UPDATE cart SET quantity = %s WHERE user_id=%s AND product_id=%s", (qty, uid, pid))
    conn.close()
...
def update_order_status(oid, status, deadline=None):
    conn = get_conn()
    c = conn.cursor()
    if deadline: c.execute("UPDATE orders SET status=%s, delivery_deadline=%s WHERE id=%s", (status, deadline, oid))
    else: c.execute("UPDATE orders SET status=%s WHERE id=%s", (status, oid))
    conn.close()
...
def plural(word, num):
    last_digit = num % 10; last_two = num % 100
...
async def start(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"🎮 {user['game_nickname']}, добро пожаловать! Выберите действие:", reply_markup=main_menu())
...
async def save_nickname(msg: Message, state: FSMContext):
    ...
    conn.close()
    await state.clear()
    await msg.answer(f"✅ Ник «{nickname}» сохранён!", reply_markup=main_menu())
...
async def admin_panel_entry(msg: Message, state: FSMContext):
    if is_admin(msg.from_user.id): await show_admin_panel(msg); return
...
async def check_admin_password(msg: Message, state: FSMContext):
    if msg.text.strip() == ADMIN_PASSWORD:
        await state.clear(); add_admin(msg.from_user.id, msg.from_user.username or "admin")
        await msg.answer("✅ Доступ разрешён!"); await show_admin_panel(msg)
    else: await msg.answer("❌ Неверный пароль!"); await state.clear()

async def show_admin_panel(msg):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🚫 Заблокировать", callback_data="admin_block_shop")],
        [InlineKeyboardButton(text="✅ Разблокировать", callback_data="admin_unblock_shop")],
...
async def do_block_shop(msg: Message, state: FSMContext):
    uid = find_user(msg.text.strip()); await state.clear()
    if uid:
        block_shop(uid); shop = get_shop(uid)
        name = shop['shop_name'] if shop else f"ID {uid}"
        await msg.answer(f"🚫 «{name}» заблокирован!"); await show_admin_panel(msg)
    else: await msg.answer("❌ Не найдено!")
...
async def do_unblock_shop(msg: Message, state: FSMContext):
    uid = find_user(msg.text.strip()); await state.clear()
    if uid:
        unblock_shop(uid); shop = get_shop(uid)
        name = shop['shop_name'] if shop else f"ID {uid}"
        await msg.answer(f"✅ «{name}» разблокирован!"); await show_admin_panel(msg)
    else: await msg.answer("❌ Не найдено!")
...
async def show_shop_stats(msg: Message, state: FSMContext):
    await state.clear(); uid = find_user(msg.text.strip())
    if not uid: await msg.answer("❌ Не найдено!"); return
    shop = get_shop(uid)
    if not shop: await msg.answer("❌ Нет магазина!"); return
    user = get_user_info(uid); stats = get_seller_stats(uid)
    nickname = user['game_nickname'] if user else "—"
    text = f"📊 <b>Магазин</b>\n\n🏪 «{shop['shop_name']}»\n👤 ID: {uid}\n🎮 Ник: {nickname}\n📧 {shop['seller_game_email']}\n🔐 {shop['shop_password']}\n\n✅ Выполнено: {stats['total_orders']}\n💰 Заработано: {stats['total_earned']}\n⏳ Активных: {stats['pending']}"
...
async def show_player_stats(msg: Message, state: FSMContext):
    await state.clear(); uid = find_user(msg.text.strip())
    if not uid: await msg.answer("❌ Не найдено!"); return
    user = get_user_info(uid); buyer = get_buyer_stats(uid); shop = get_shop(uid)
    nickname = user['game_nickname'] if user else "—"
...
async def admin_blocked_list(cb: CallbackQuery):
    ...
    if not blocked: await cb.message.edit_text("Нет заблокированных.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_admin")]])); return
    text = "🚫 <b>Заблокированные:</b>\n\n"
    for b in blocked: text += f"• ID: {b['user_id']} | {b['shop_name']}\n"
...
async def admin_add_admin(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id): await cb.answer("❌ Нет доступа!"); return
    await cb.message.edit_text("Введите user_id:"); await state.set_state(AdminStates.waiting_for_add_admin)

@router.message(AdminStates.waiting_for_add_admin)
async def do_add_admin(msg: Message, state: FSMContext):
    try:
        uid = int(msg.text.strip()); add_admin(uid, "admin"); await state.clear()
        await msg.answer(f"✅ Админ {uid} добавлен!"); await show_admin_panel(msg)
    except: await msg.answer("❌ Ошибка!"); await state.clear()

@router.callback_query(F.data == "admin_remove_admin")
async def admin_remove_admin(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id): await cb.answer("❌ Нет доступа!"); return
    await cb.message.edit_text("Введите user_id:"); await state.set_state(AdminStates.waiting_for_remove_admin)
...
async def do_remove_admin(msg: Message, state: FSMContext):
    try:
        ...
        remove_admin(uid); await state.clear()
        await msg.answer(f"✅ Админ {uid} удалён!"); await show_admin_panel(msg)
    except: await msg.answer("❌ Ошибка!"); await state.clear()
...
async def all_tasks(cb: CallbackQuery):
    ...
    if not tasks: await cb.message.edit_text("📋 Нет активных заданий.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def task_detail(cb: CallbackQuery):
    tid = int(cb.data.split("_")[2]); t = get_task(tid)
    if not t: await cb.answer("Задание не найдено"); return
    seller = get_shop(t['seller_id']); contact = seller['seller_game_email'] if seller else "—"
    ...
    if t['seller_id'] == cb.from_user.id:
        ...
        if t['status'] != 'closed': kb.append([InlineKeyboardButton(text="🔒 Закрыть", callback_data=f"close_task_{tid}")])
...
async def take_task(cb: CallbackQuery, bot: Bot):
    ...
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE tasks SET status='taken', buyer_id=%s WHERE id=%s", (cb.from_user.id, tid)); conn.close()
...
async def create_task_start(cb: CallbackQuery, state: FSMContext):
    await cb.message.edit_text("📝 Опишите задание:"); await state.set_state(SellerStates.task_description)
...
async def task_desc(msg: Message, state: FSMContext):
    ...
    await msg.answer("💰 Введите цену (0 = договорная):"); await state.set_state(SellerStates.task_price)
...
async def task_currency(msg: Message, state: FSMContext):
    ...
    await msg.answer("📧 Ваша почта:"); await state.set_state(SellerStates.task_deadline)
...
async def task_deadline(msg: Message, state: FSMContext, bot: Bot):
    ...
    if data.get('task_description'):
        ...
        await state.clear(); await msg.answer("✅ Задание создано!"); await task_board(msg)
        ...
        t = get_task(tid); await state.clear(); await msg.answer("✅ Отмечено выполненным!")
        if t:
...
async def task_responses(cb: CallbackQuery):
    tid = int(cb.data.split("_")[2]); responses = get_task_responses(tid)
    if not responses: await cb.message.edit_text("👀 Нет откликов.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    for r in responses:
        ...
        if r['status'] == 'pending': kb.append([InlineKeyboardButton(text=f"✅ Выбрать @{r['username']}", callback_data=f"accept_response_{r['id']}_{tid}")])
...
async def accept_response(cb: CallbackQuery, bot: Bot):
    ...
    conn.close(); accept_task_response(rid, tid); await cb.answer("✅ Выбран!")
    conn = get_conn(); c = conn.cursor(cursor_factory=RealDictCursor)
...
async def close_task_cb(cb: CallbackQuery):
    tid = int(cb.data.split("_")[2]); close_task(tid); await cb.answer("🔒 Закрыто!"); await my_tasks(cb)

@router.callback_query(F.data == "my_tasks")
async def my_tasks(cb: CallbackQuery):
    tasks = get_my_tasks(cb.from_user.id)
    if not tasks: await cb.message.edit_text("📋 Нет заданий.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data="task_board")]])); return
...
async def help_cmd(msg: Message):
    text = (
        ...
        f"📩 <b>Связь:</b> {SUPPORT_USERNAME}\n💡 <b>Идеи:</b> {SUPPORT_USERNAME}"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Главное меню", callback_data="start_menu")],
        [InlineKeyboardButton(text="📩 Написать", url=f"https://t.me/{SUPPORT_USERNAME.replace('@','')}")]])
...
async def shops_list(cb: CallbackQuery):
    ...
    kb = []
    for s in shops:
        if not is_shop_blocked(s['user_id']): kb.append([InlineKeyboardButton(text=f"🏪 {s['shop_name']}", callback_data=f"shop_{s['user_id']}")])
...
async def show_all_products(cb: CallbackQuery):
    seller_id = int(cb.data.split("_")[1]); prods = get_products(seller_id=seller_id)
...
async def show_products(cb: CallbackQuery):
    cat_id = int(cb.data.split("_")[1]); prods = get_products(cat_id=cat_id)
    conn = get_conn(); c = conn.cursor(cursor_factory=RealDictCursor)
    c.execute("SELECT seller_id FROM categories WHERE id=%s", (cat_id,))
    cat = c.fetchone(); conn.close(); seller_id = cat['seller_id'] if cat else 0
    if not prods: await cb.message.edit_text("Нет товаров.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data=f"shop_{seller_id}")]])); return
    kb = []
    for p in prods:
...
async def buy_qty_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(txt); await state.set_state(SellerStates.cart_input_qty)
...
async def buy_qty_done(msg: Message, state: FSMContext):
    ...
    disc_price, _, _ = get_discounted_price(pid); price = disc_price if disc_price else p['price']
    curr = plural(p['currency'], price * qty)
    await msg.answer(f"✅ «{p['name']}» x{qty} = {price*qty} {curr}"); await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🛒 Корзина", callback_data="view_cart")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data=f"shop_{p['seller_id']}")]])
...
async def view_cart(cb: CallbackQuery):
    ...
    for i in items:
        disc_price, _, _ = get_discounted_price(i['id']); price = disc_price if disc_price else i['price']
...
async def checkout(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("📧 Введите игровую почту:"); await state.set_state(OrderStates.waiting_for_email)
...
async def cancel_order_cb(cb: CallbackQuery, bot: Bot):
    ...
    cancel_order(oid); await bot.send_message(order['seller_id'], f"❌ Заказ №{oid} отменён.")
...
async def seller_menu(cb: CallbackQuery):
    ...
    for s in get_all_shops(): kb.append([InlineKeyboardButton(text=f"🏪 {s['shop_name']}", callback_data=f"login_shop_{s['user_id']}")])
...
async def login_shop(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(login_shop_id=shop_id); login_attempts[cb.from_user.id] = 0
...
async def seller_orders(cb: CallbackQuery):
    ...
    for oid in orders:
        ...
        if o:
            ...
            elif o['status'] == 'accepted': kb.append([InlineKeyboardButton(text=f"🎉 Готов №{oid}", callback_data=f"ready_{oid}")])
...
async def accept_order(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(oid=oid); await cb.message.edit_text(f"⏳ Срок выполнения заказа №{oid}:")
...
async def manage_discounts(cb: CallbackQuery):
    prods = get_products(seller_id=cb.from_user.id); discounts = get_seller_discounts(cb.from_user.id)
    text = "🏷 <b>Скидки:</b>\n\n"
    if discounts:
