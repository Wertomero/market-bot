...
def init_db():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS products (id INTEGER PRIMARY KEY AUTOINCREMENT, category_id INTEGER, seller_id INTEGER, name TEXT, description TEXT, price INTEGER, currency TEXT, stock INTEGER DEFAULT 1)")
...
def search_products(query):
    conn = get_conn()
    prods = [dict(r) for r in conn.execute("SELECT * FROM products WHERE name LIKE ? AND stock > 0", (f'%{query}%',)).fetchall()]
    conn.close()
    return prods
...
def get_pending_orders(seller_id):
    conn = get_conn()
    orders = [r['id'] for r in conn.execute("SELECT id FROM orders WHERE seller_id=? AND status IN ('pending','accepted')", (seller_id,)).fetchall()]
    conn.close()
    return orders
...
def get_buyer_orders(buyer_id):
    conn = get_conn()
    orders = [dict(r) for r in conn.execute("SELECT * FROM orders WHERE buyer_id=? ORDER BY id DESC LIMIT 20", (buyer_id,)).fetchall()]
    conn.close()
    return orders
...
async def search_start(cb: CallbackQuery, state: FSMContext):
    await cb.message.edit_text("🔍 Введите название товара для поиска:")
...
async def search_result(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"🔍 Результаты поиска «{query}»:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def back_to_shop(cb: CallbackQuery):
    ...
    conn.close()
    if r:
        cats = get_categories(r['seller_id'])
        kb = []
        for cat in cats:
            kb.append([InlineKeyboardButton(text=f"📁 {cat['name']}", callback_data=f"cat_{cat['id']}")])
        kb.append([InlineKeyboardButton(text="🔙 К магазинам", callback_data="buyer")])
