...
def get_top_shops(limit=10):
    ...
    c = conn.cursor(cursor_factory=RealDictCursor)
    c.execute("""
        SELECT u.user_id, u.shop_name, u.username, COALESCE(SUM(o.total_amount),0) as earned
        FROM users u
        LEFT JOIN orders o ON u.user_id = o.seller_id AND o.status = 'ready'
        WHERE u.shop_name IS NOT NULL
        GROUP BY u.user_id, u.shop_name, u.username
...
async def buy_qty_done(msg: Message, state: FSMContext):
    ...
    if p['pack_qty'] > 1:
        await msg.answer(f"✅ «{p['name']}» x{qty} уп. ({total_qty} шт) = {p['price']*qty} {curr}")
    else:
        await msg.answer(f"✅ «{p['name']}» x{qty} = {p['price']*qty} {curr}")
    await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
