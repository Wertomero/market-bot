...
# Все старые функции (add_user...cancel_order) остаются без изменений
...
async def show_admin_panel(msg):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="👤 Статистика игрока", callback_data="admin_view_player")],
...
async def admin_block_shop(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("Введите user_id магазина для блокировки:")
...
async def do_block_shop(msg: Message, state: FSMContext):
    try:
        ...
        await msg.answer("❌ Ошибка!"); await state.clear()

@router.callback_query(F.data == "admin_unblock_shop")
async def admin_unblock_shop(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id): await cb.answer("❌ Нет доступа!"); return
    await cb.message.edit_text("Введите user_id магазина для разблокировки:")
...
async def admin_view_shop(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id): await cb.answer("❌ Нет доступа!"); return
    await cb.message.edit_text("Введите user_id магазина:")
...
async def admin_blocked_list(cb: CallbackQuery):
    ...
    if not blocked:
        await cb.message.edit_text("Нет заблокированных магазинов.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def take_task(cb: CallbackQuery, bot: Bot):
    ...
    if not t or t['status'] != 'active':
        await cb.answer("❌ Задание недоступно!"); return
    if t['seller_id'] == cb.from_user.id:
        await cb.answer("❌ Нельзя взять своё задание!"); return
    conn = get_conn()
    c = conn.cursor()
    ...
    try:
        await bot.send_message(t['seller_id'], f"🔔 <b>Задание #{tid} взято!</b>\n👤 Исполнитель: @{cb.from_user.username or cb.from_user.id} (ID: {cb.from_user.id})", parse_mode="HTML")
    except: pass
...
async def complete_task(cb: CallbackQuery, state: FSMContext):
    tid = int(cb.data.split("_")[2])
    t = get_task(tid)
    if not t or t.get('buyer_id') != cb.from_user.id:
        await cb.answer("❌ Не ваш заказ!"); return
...
async def task_desc(msg: Message, state: FSMContext):
    ...
    await msg.answer("💰 Введите цену (или 0 если договорная):")
...
async def task_deadline(msg: Message, state: FSMContext, bot: Bot):
    ...
    if data.get('task_description'):
        ...
        tid = data['complete_tid']
        email = msg.text.strip()
        conn = get_conn(); c = conn.cursor()
        c.execute("UPDATE tasks SET status='completed' WHERE id=%s", (tid,))
        conn.close()
        t = get_task(tid)
        await state.clear()
        await msg.answer("✅ Задание отмечено выполненным!")
        if t:
            try:
                await bot.send_message(t['seller_id'], f"🎉 <b>Задание #{tid} выполнено!</b>\n👤 Исполнитель: @{msg.from_user.username or '—'} (ID: {msg.from_user.id})\n📧 Почта: {email}", parse_mode="HTML")
            except: pass
...
async def task_responses(cb: CallbackQuery):
    ...
    if not responses:
        await cb.answer("👀 Пока нет откликов")
        await cb.message.edit_text("👀 <b>Отклики на задание:</b>\n\nПока никто не откликнулся.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data=f"view_task_{tid}")]]))
        return
    text = "👀 <b>Отклики:</b>\n\n"
    kb = []
    for r in responses:
...
async def accept_response(cb: CallbackQuery, bot: Bot):
    ...
    for r in responses:
        if r['id'] == rid:
            try: await bot.send_message(r['buyer_id'], f"✅ Вас выбрали исполнителем задания #{tid}!")
            except: pass
        elif r['status'] == 'pending':
            try: await bot.send_message(r['buyer_id'], f"❌ Ваш отклик на задание #{tid} отклонён.")
            except: pass
    await cb.message.edit_text("✅ Исполнитель выбран!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 К моим заданиям", callback_data="my_tasks")]]))

@router.callback_query(F.data.startswith("close_task_"))
async def close_task_cb(cb: CallbackQuery):
    tid = int(cb.data.split("_")[2])
    close_task(tid)
    await cb.answer("🔒 Задание закрыто!")
    await my_tasks(cb)
...
async def my_tasks(cb: CallbackQuery):
    ...
    if not tasks:
        await cb.message.edit_text("📋 У вас нет заданий.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data="task_board")]]))
        return
...
async def help_cmd(msg: Message):
    await msg.answer(f"📖 <b>Помощь по боту</b>\n\n🛒 <b>Покупатель:</b>\n• Смотрите магазины, категории и товары\n• 🔍 Поиск товаров и магазинов\n• Вводите количество для покупки\n• Оформляйте заказы\n\n🏪 <b>Продавец:</b>\n• Создайте магазин с паролем\n• Добавляйте товары с упаковками\n• 🏷 Скидки на товары\n• 📊 Статистика в управлении\n\n📋 <b>Доска заданий:</b>\n• Создавайте задания\n• Берите задания других\n• Отмечайте выполнение\n\n🏆 <b>Топ продавцов</b>\n📊 <b>Моя статистика</b>\n\n📩 Связь: {SUPPORT_USERNAME}", parse_mode="HTML")

@router.callback_query(F.data == "help")
async def help_cb(cb: CallbackQuery):
    await cb.message.edit_text(f"📖 <b>Помощь по боту</b>\n\n🛒 <b>Покупатель:</b>\n• Смотрите магазины, категории и товары\n• 🔍 Поиск товаров и магазинов\n• Вводите количество для покупки\n• Оформляйте заказы\n\n🏪 <b>Продавец:</b>\n• Создайте магазин с паролем\n• Добавляйте товары с упаковками\n• 🏷 Скидки на товары\n• 📊 Статистика в управлении\n\n📋 <b>Доска заданий:</b>\n• Создавайте задания\n• Берите задания других\n• Отмечайте выполнение\n\n🏆 <b>Топ продавцов</b>\n📊 <b>Моя статистика</b>\n\n📩 Связь: {SUPPORT_USERNAME}", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data="start_menu")]]))
...
async def top_shops(cb: CallbackQuery):
    ...
    if not shops:
        await cb.message.edit_text("😔 Пока нет продавцов.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def shops_list(cb: CallbackQuery):
    ...
    if not kb:
        await cb.message.edit_text("😔 Нет доступных магазинов.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data="buyer")]])); return
...
async def search_shop_start(cb: CallbackQuery, state: FSMContext):
    await cb.message.edit_text("🔍 Введите название магазина:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def open_shop(cb: CallbackQuery):
    ...
    if is_shop_blocked(seller_id):
        await cb.answer("🚫 Магазин заблокирован!"); return
...
async def show_products(cb: CallbackQuery):
    ...
    if not prods:
        await cb.message.edit_text("Нет товаров.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 К категориям", callback_data=f"shop_{seller_id}")]])); return
    kb = []
    for p in prods:
        disc_price, disc_percent, _ = get_discounted_price(p['id'])
        if disc_price:
...
async def product_detail(cb: CallbackQuery):
    ...
    p = get_product(pid)
    if not p: await cb.answer("Товар не найден"); return
    if is_shop_blocked(p['seller_id']):
        await cb.answer("🚫 Магазин заблокирован!"); return
    ...
    if disc_price:
        ...
        if p['pack_qty'] > 1:
            text = f"<b>{p['name']}</b>\n📦 Упаковка: <b>{p['pack_qty']} шт</b>\n💰 Цена за упаковку: <b>{p['price']} {curr}</b>\n📦 В наличии: {p['stock']} упаковок"
        else:
            text = f"<b>{p['name']}</b>\n💰 Цена: {p['price']} {curr}\n📦 В наличии: {p['stock']} шт"
...
async def buy_qty_done(msg: Message, state: FSMContext):
    ...
    if qty <= 0: await msg.answer("❌ Больше нуля!"); return
    if qty > p['stock']: await msg.answer(f"❌ В наличии только {p['stock']}!"); return
    cart = get_cart(msg.from_user.id)
    if cart and cart[0]['seller_id'] != p['seller_id']:
        await msg.answer("❌ Очистите корзину!"); await state.clear(); return
...
async def edit_cart_start(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(buy_pid=pid)
    if p['pack_qty'] > 1:
        await cb.message.edit_text(f"📦 Новое кол-во упаковок для «{p['name']}» (0 = удалить):")
    else:
...
async def checkout(cb: CallbackQuery, state: FSMContext):
    items = get_cart(cb.from_user.id)
    if not items: await cb.answer("Корзина пуста!"); return
...
async def process_order(msg: Message, state: FSMContext, bot: Bot):
    ...
    for i in items:
        ...
        price = disc_price if disc_price else i['price']
        total += price * i['quantity']
    if not items: await msg.answer("Корзина пуста!"); await state.clear(); return
    ...
    await msg.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отменить заказ", callback_data=f"cancel_{oid}")]]), parse_mode="HTML")
    await state.clear()
    ...
    for i in order['items']:
        ...
        curr = plural(i['currency'], price * i['quantity'])
        if i['pack_qty'] > 1:
            stext += f"• {i['product_name']} (x{i['pack_qty']}) x{i['quantity']} уп. = {price*i['quantity']} {curr} ({total_qty} шт)\n"
        else:
            stext += f"• {i['product_name']} x{i['quantity']} = {price*i['quantity']} {curr}\n"
    stext += f"\n💰 Итого: <b>{total}</b>"
    try:
...
async def cancel_order_cb(cb: CallbackQuery, bot: Bot):
    ...
    order = get_order(oid)
    if order['buyer_id'] != cb.from_user.id: await cb.answer("❌ Не ваш заказ!"); return
    if order['status'] != 'pending': await cb.answer("❌ Нельзя отменить!"); return
    cancel_order(oid)
...
async def order_detail(cb: CallbackQuery):
    oid = int(cb.data.split("_")[1])
    order = get_order(oid)
    if not order: await cb.answer("Заказ не найден"); return
    emoji = {"pending": "⏳", "accepted": "✅", "ready": "🎉", "rejected": "❌", "cancelled": "🚫"}
    text = f"📋 <b>Заказ №{oid}</b>\nСтатус: {emoji.get(order['status'], '?')} {order['status']}\n💰 Сумма: {order['total_amount']}\n📧 Почта: {order['buyer_game_email']}\n"
    if order['delivery_deadline']: text += f"⏳ Срок: {order['delivery_deadline']}\n"
    text += f"\n📦 <b>Товары:</b>\n"
    for i in order['items']:
...
async def create_shop(cb: CallbackQuery, state: FSMContext):
    if has_shop(cb.from_user.id): await cb.answer("❌ У вас уже есть магазин!"); return
...
async def login_shop(cb: CallbackQuery, state: FSMContext):
    ...
    if is_shop_blocked(shop_id):
        await cb.answer("🚫 Магазин заблокирован!"); return
    shop = get_shop(shop_id)
    if not shop: await cb.answer("Магазин не найден"); return
...
async def check_password(msg: Message, state: FSMContext):
    ...
    if shop:
        ...
        if attempts >= 3:
            login_attempts[msg.from_user.id] = 0
            await msg.answer("🚫 3 неверные попытки."); await state.clear()
        else:
...
async def seller_inside_msg(msg, shop_id):
    shop = get_shop(shop_id)
    if not shop: await msg.answer("Магазин не найден"); return
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def accept_order(cb: CallbackQuery, state: FSMContext, bot: Bot):
    oid = int(cb.data.split("_")[1])
    order = get_order(oid)
    if order['seller_id'] != cb.from_user.id: await cb.answer("❌ Не ваш заказ!"); return
    if order['status'] != 'pending': await cb.answer("❌ Заказ уже не активен!"); return
...
async def deadline_done(msg: Message, state: FSMContext, bot: Bot):
    ...
    order = get_order(oid)
    await bot.send_message(order['buyer_id'], f"✅ <b>Заказ №{oid} принят!</b>\n⏳ Срок: <b>{dl}</b>\n💰 Сумма: <b>{order['total_amount']}</b>", parse_mode="HTML")
...
async def ready_order(cb: CallbackQuery, bot: Bot):
    ...
    order = get_order(oid)
    if order['seller_id'] != cb.from_user.id: await cb.answer("❌ Не ваш заказ!"); return
    if order['status'] != 'accepted': await cb.answer("❌ Заказ уже не активен!"); return
    semail = get_seller_email(cb.from_user.id)
    update_order_status(oid, 'ready')
    text = f"🎉 <b>Заказ №{oid} готов!</b>\n\n📦 Товары:\n"
    for i in order['items']:
        if i['pack_qty'] > 1: text += f"• {i['product_name']} (x{i['pack_qty']}) x{i['quantity']} уп.\n"
        else: text += f"• {i['product_name']} x{i['quantity']}\n"
    ...
    await bot.send_message(order['seller_id'], f"🎉 Заказ №{oid} готов.", parse_mode="HTML")
...
async def edit_product_start(cb: CallbackQuery):
    ...
    for p in prods:
        ...
        if p['pack_qty'] > 1: text = f"✏️ {p['name']} (x{p['pack_qty']}) — {p['price']} {curr} ({p['stock']} уп)"
...
async def edit_product_menu(cb: CallbackQuery):
    pid = int(cb.data.split("_")[1])
    p = get_product(pid)
    if not p or p['seller_id'] != cb.from_user.id: await cb.answer("❌ Не ваш товар!"); return
    curr = plural(p['currency'], p['price'])
    text = f"<b>{p['name']}</b>\n💰 Цена: {p['price']} {curr}\n📦 На складе: {p['stock']}"
...
async def del_product_start(cb: CallbackQuery):
    ...
    for p in prods:
        ...
        if p['pack_qty'] > 1: text = f"🗑 {p['name']} (x{p['pack_qty']}) — {p['price']} {curr} ({p['stock']} уп)"
...
async def product_description(msg: Message, state: FSMContext):
    ...
    await msg.answer("📦 Введите количество товара в одной упаковке (1 = поштучно):")
    await state.set_state(SellerStates.adding_product_pack_qty)
...
async def product_pack_qty(msg: Message, state: FSMContext):
    try:
        ...
        if pack_qty <= 0: await msg.answer("❌ Больше нуля!"); return
    except: await msg.answer("❌ Число!"); return
    await state.update_data(prod_pack_qty=pack_qty)
    if pack_qty > 1: await msg.answer(f"💰 Введите цену за упаковку из {pack_qty} шт:")
    else: await msg.answer("💰 Введите цену:")
...
async def product_currency(msg: Message, state: FSMContext):
    ...
    await msg.answer("📦 Введите количество на складе:")
...
async def product_stock(msg: Message, state: FSMContext):
    ...
    pack_qty = data.get('prod_pack_qty', 1)
    curr = plural(data['prod_currency'], data['prod_price'])
    await state.clear()
    if pack_qty > 1: await msg.answer(f"✅ «{data['prod_name']}» (x{pack_qty}) за {data['prod_price']} {curr} ({stock} уп) добавлен!")
    else: await msg.answer(f"✅ «{data['prod_name']}» за {data['prod_price']} {curr} ({stock} шт) добавлен!")
    await seller_inside_msg(msg, msg.from_user.id)
...
async def manage_discounts(cb: CallbackQuery):
    ...
    else: text += "Нет активных скидок.\n"
    kb = []
    for p in prods: kb.append([InlineKeyboardButton(text=f"🏷 {p['name']}", callback_data=f"setdiscount_{p['id']}")])
...
async def set_discount_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("💰 Введите процент скидки:")
...
async def discount_percent_done(msg: Message, state: FSMContext):
    try:
        ...
        if percent <= 0 or percent > 99: await msg.answer("❌ От 1 до 99!"); return
    except: await msg.answer("❌ Число!"); return
    await state.update_data(discount_percent=percent)
    await msg.answer("⏳ На сколько часов?")
...
async def discount_hours_done(msg: Message, state: FSMContext):
    try:
        ...
        if hours <= 0: await msg.answer("❌ Больше нуля!"); return
    except: await msg.answer("❌ Число!"); return
    data = await state.get_data()
...
async def remove_discount_menu(cb: CallbackQuery):
    discounts = get_seller_discounts(cb.from_user.id)
    if not discounts: await cb.answer("Нет активных скидок!"); return
    kb = []
    for d in discounts: kb.append([InlineKeyboardButton(text=f"❌ {d['name']} ({d['percent']}%)", callback_data=f"removediscount_{d['product_id']}")])
