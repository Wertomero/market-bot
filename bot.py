...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS users (
        user_id BIGINT PRIMARY KEY, username TEXT, shop_name TEXT, seller_game_id TEXT)""")
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS orders (
        id SERIAL PRIMARY KEY, buyer_id BIGINT, seller_id BIGINT, status TEXT DEFAULT 'pending', total_amount INT, buyer_game_id TEXT, seller_game_id TEXT, delivery_deadline TEXT)""")
...
def set_shop(uid, shop_name, game_id):
    conn = get_conn()
    c = conn.cursor()
    c.execute("UPDATE users SET shop_name=%s, seller_game_id=%s WHERE user_id=%s", (shop_name, game_id, uid))
    conn.close()
...
def get_shop(uid):
    conn = get_conn()
    c = conn.cursor(cursor_factory=RealDictCursor)
    c.execute("SELECT shop_name, seller_game_id FROM users WHERE user_id=%s", (uid,))
    r = c.fetchone()
    conn.close()
...
def get_seller_game_id(uid):
    conn = get_conn()
    c = conn.cursor(cursor_factory=RealDictCursor)
    c.execute("SELECT seller_game_id FROM users WHERE user_id=%s", (uid,))
    r = c.fetchone()
    conn.close()
...
def update_cart(uid, pid, delta):
    conn = get_conn()
    c = conn.cursor()
    c.execute("UPDATE cart SET quantity = quantity + %s WHERE user_id=%s AND product_id=%s", (delta, uid, pid))
    c.execute("DELETE FROM cart WHERE user_id=%s AND product_id=%s AND quantity <= 0", (uid, pid))
    conn.close()
...
def create_order(buyer_id, seller_id, total, buyer_gid, items):
    ...
    c = conn.cursor()
    c.execute("INSERT INTO orders (buyer_id, seller_id, total_amount, buyer_game_id, seller_game_id) VALUES (%s,%s,%s,%s,%s) RETURNING id", (buyer_id, seller_id, total, buyer_gid, sgid))
    oid = c.fetchone()[0]
    for item in items:
...
async def help_cmd(msg: Message):
    await msg.answer(
        ...
        "🏪 <b>Продавец:</b>\n"
        "• Создайте один магазин\n"
        "• Добавляйте категории и товары\n"
        "• Указывайте цену, валюту, количество\n"
        "• Принимайте или отклоняйте заказы\n"
        "• Указывайте срок выполнения\n"
        "• Нажмите «Готов», когда заказ выполнен\n\n"
        "💰 Оплата происходит вне бота — покупатель сам переводит валюту продавцу.",
        parse_mode="HTML"
    )
...
async def product_detail(cb: CallbackQuery):
    ...
    kb.append([InlineKeyboardButton(text="🔙 К товарам", callback_data=f"shop_{p['seller_id']}")])
...
async def shop_name(msg: Message, state: FSMContext):
    ...
    await msg.answer("🎮 Ваш игровой ID (куда покупатели будут переводить валюту):")
...
async def login_shop(cb: CallbackQuery):
    ...
    if cb.from_user.id == shop_id:
        await cb.message.edit_text(f"✅ Добро пожаловать в «{shop['shop_name']}»!")
        await seller_inside(cb.message)
        return
    await cb.message.edit_text(f"🏪 «{shop['shop_name']}»\nВы можете только смотреть товары.")
    await open_shop(cb)
...
async def seller_inside(msg):
    ...
    await msg.answer(f"🏪 «{shop['shop_name']}»\n🎮 ID: {shop['seller_game_id']}", reply_markup=kb)
