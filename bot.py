...
async def my_stats(cb: CallbackQuery):
    ...
    if shop:
        ...
        text += (
            ...
            f"• Выполнено: <b>{seller['total_orders']}</b>\n"
            f"• Заработано: <b>{seller['total_earned']}</b>\n"
            f"• Активных: <b>{seller['pending']}</b>"
        )
    else:
...
async def search_result(msg: Message, state: FSMContext):
    ...
    for p in prods:
        ...
        if disc_price:
            ...
            text_btn = f"{p['name']} — 🔥 {disc_price} {curr} (-{disc_percent}%)"
        else:
            curr = plural(p['currency'], p['price'])
            text_btn = f"{p['name']} — {p['price']} {curr}"
        if p['pack_qty'] > 1:
            text_btn = f"{p['name']} (x{p['pack_qty']}) — {text_btn.split(' — ')[1]}"
        kb.append([InlineKeyboardButton(text=f"{text_btn} ({p['stock']} шт)", callback_data=f"prod_{p['id']}")])
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data="buyer")])
    await msg.answer("🔍 Результаты:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def edit_product_start(cb: CallbackQuery):
    ...
    for p in prods:
        curr = plural(p['currency'], p['price'])
        text_btn = f"✏️ {p['name']} — {p['price']} {curr} ({p['stock']})"
        if p['pack_qty'] > 1:
            text_btn = f"✏️ {p['name']} (x{p['pack_qty']}) — {p['price']} {curr} ({p['stock']} уп)"
        kb.append([InlineKeyboardButton(text=text_btn, callback_data=f"editprod_{p['id']}")])
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data=f"back_to_shop_{cb.from_user.id}")])
...
async def del_product_start(cb: CallbackQuery):
    ...
    for p in prods:
        curr = plural(p['currency'], p['price'])
        text_btn = f"🗑 {p['name']} — {p['price']} {curr} ({p['stock']})"
        if p['pack_qty'] > 1:
            text_btn = f"🗑 {p['name']} (x{p['pack_qty']}) — {p['price']} {curr} ({p['stock']} уп)"
        kb.append([InlineKeyboardButton(text=text_btn, callback_data=f"delprod_{p['id']}")])
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data=f"back_to_shop_{cb.from_user.id}")])
    await cb.message.edit_text("Выберите товар:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
