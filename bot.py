...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS tasks (
        id SERIAL PRIMARY KEY, seller_id BIGINT, description TEXT, price INT, currency TEXT, deadline TEXT, status TEXT DEFAULT 'active', created_at TIMESTAMP DEFAULT NOW())""")
...
def add_task(seller_id, description, price, currency, deadline):
    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT INTO tasks (seller_id, description, price, currency, deadline) VALUES (%s,%s,%s,%s,%s)",
              (seller_id, description, price, currency, deadline))
    conn.close()
...
def add_task_response(task_id, buyer_id, price, currency):
    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT INTO task_responses (task_id, buyer_id, price, currency) VALUES (%s,%s,%s,%s)",
              (task_id, buyer_id, price, currency))
    conn.close()
...
class SellerStates(StatesGroup):
    ...
    task_currency = State()
    task_deadline = State()
    task_response_price = State()
    task_response_currency = State()
...
async def create_task_start(cb: CallbackQuery, state: FSMContext):
    await cb.message.edit_text("📝 Опишите задание (что нужно сделать/принести):")
...
async def task_deadline(msg: Message, state: FSMContext, bot: Bot):
    ...
    # Проверяем, это создание задания или завершение
    if data.get('task_description'):
        ...
        t = get_task(tid)
        await state.clear()
        await msg.answer("✅ Задание отмечено выполненным! Продавец получит уведомление.")
...
async def task_responses(cb: CallbackQuery):
    ...
    if not responses:
        ...
        await cb.message.edit_text(
            "👀 <b>Отклики на задание:</b>\n\nПока никто не откликнулся.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Назад", callback_data=f"task_{tid}")]
            ])
        )
        return
    
    text = "👀 <b>Отклики на задание:</b>\n\n"
    kb = []
    for r in responses:
    ...
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data=f"task_{tid}")])
    await cb.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def accept_response(cb: CallbackQuery, bot: Bot):
    ...
    for r in responses:
        if r['id'] == rid:
            ...
            try:
                await bot.send_message(r['buyer_id'], f"❌ Ваш отклик на задание #{tid} отклонён.")
            except:
                pass
...
async def help_cmd(msg: Message):
    await msg.answer(
        ...
        f"📋 <b>Доска заданий:</b>\n"
        f"• Создавайте задания с ценой или без\n"
        f"• Откликайтесь на задания других\n"
        f"• Выбирайте исполнителей\n\n"
        f"🏆 <b>Топ продавцов</b> — рейтинг по заработку\n"
...
async def my_stats(cb: CallbackQuery):
    ...
    text += f"• Куплено заказов: <b>{buyer['total_bought']}</b>\n"
    text += f"• Потрачено всего: <b>{buyer['total_spent']}</b>\n"
    text += f"• Активных заказов: <b>{buyer['pending']}</b>\n"
    shop = get_shop(cb.from_user.id)
    if shop:
        seller = get_seller_stats(cb.from_user.id)
        text += f"\n🏪 <b>Как продавец ({shop['shop_name']}):</b>\n"
        text += f"• Выполнено заказов: <b>{seller['total_orders']}</b>\n"
        text += f"• Заработано: <b>{seller['total_earned']}</b>\n"
        text += f"• Активных заказов: <b>{seller['pending']}</b>"
    else:
...
async def search_shop_result(msg: Message, state: FSMContext):
    ...
    for s in shops:
        kb.append([InlineKeyboardButton(text=f"🏪 {s['shop_name']}", callback_data=f"shop_{s['user_id']}")])
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data="buyer")])
    await msg.answer(f"🔍 Результаты:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def buy_qty_start(cb: CallbackQuery, state: FSMContext):
    ...
    if p['pack_qty'] > 1:
        await cb.message.edit_text(f"📦 Введите количество упаковок для «{p['name']}» (в уп. {p['pack_qty']} шт, на складе: {p['stock']} уп.):", parse_mode="HTML")
    else:
...
async def deadline_done(msg: Message, state: FSMContext, bot: Bot):
    ...
    await bot.send_message(order['seller_id'],
        f"📥 Вы приняли заказ №{oid}. Срок: {dl}", parse_mode="HTML")
...
async def product_pack_qty(msg: Message, state: FSMContext):
    ...
    if pack_qty > 1:
        await msg.answer(f"💰 Введите цену за упаковку из {pack_qty} шт (число):")
    else:
...
async def product_currency(msg: Message, state: FSMContext):
    ...
    await msg.answer("📦 Введите количество упаковок на складе (число):")
...
async def product_stock(msg: Message, state: FSMContext):
    ...
    await state.clear()
    if pack_qty > 1:
        await msg.answer(f"✅ «{data['prod_name']}» (x{pack_qty}) за {data['prod_price']} {curr} ({stock} уп) добавлен!")
    else:
