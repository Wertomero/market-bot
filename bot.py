...
async def help_cmd(msg: Message):
    await msg.answer(
        ...
        f"• Добавляйте товары в корзину\n"
        f"• Оформляйте заказы\n"
        f"• Отменяйте заказы в «Мои заказы»\n\n"
        f"🏪 <b>Продавец:</b>\n"
        f"• Создайте один магазин\n"
        f"• Добавляйте категории и товары\n"
        f"• Указывайте цену, валюту, количество\n"
        f"• Принимайте или отклоняйте заказы\n"
        f"• Указывайте срок выполнения\n"
        f"• Нажмите «Готов», когда заказ выполнен\n\n"
        f"💰 Оплата происходит вне бота.\n\n"
...
async def login_shop(cb: CallbackQuery):
    ...
    for cat in cats:
        kb.append([InlineKeyboardButton(text=f"📁 {cat['name']}", callback_data=f"cat_{cat['id']}")])
    kb.append([InlineKeyboardButton(text="📦 Все товары", callback_data=f"all_{shop_id}")])
    kb.append([InlineKeyboardButton(text="🔙 К магазинам", callback_data="buyer")])
    await cb.message.edit_text("📁 Выберите категорию:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def seller_inside_back(cb: CallbackQuery):
    ...
    await cb.message.edit_text("🏪 Управление:", reply_markup=kb)
