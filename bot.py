...
def create_order(buyer_id, seller_id, total, buyer_email, items):
    ...
    oid = c.fetchone()[0]
    for item in items:
        c.execute("INSERT INTO order_items (order_id, product_name, quantity, price, currency, pack_qty) VALUES (%s,%s,%s,%s,%s,%s)", (oid, item['name'], item['quantity'], item['price'], item['currency'], item['pack_qty']))
...
async def help_cmd(msg: Message):
    await msg.answer(
        ...
        f"📊 <b>Моя статистика</b> — ваши покупки и продажи\n\n"
...
async def buyer(cb: CallbackQuery):
    ...
    for s in shops:
        kb.append([InlineKeyboardButton(text=f"🏪 {s['shop_name']}", callback_data=f"shop_{s['user_id']}")])
    kb.append([InlineKeyboardButton(text="🏪 Все магазины", callback_data="shops_list")])
    kb.append([InlineKeyboardButton(text="🔍 Поиск магазинов", callback_data="search_shop")])
...
async def search_result(msg: Message, state: FSMContext):
    ...
    for p in prods:
        if p['pack_qty'] > 1:
            text = f"{p['name']} (x{p['pack_qty']}) — {p['price']} {plural(p['currency'], p['price'])}"
        else:
            text = f"{p['name']} — {p['price']} {plural(p['currency'], p['price'])}"
...
async def show_all_products(cb: CallbackQuery):
    ...
    for p in prods:
        ...
        if p['pack_qty'] > 1:
            text = f"{p['name']} (x{p['pack_qty']}) — {p['price']} {curr}"
        else:
...
async def buy_qty_start(cb: CallbackQuery, state: FSMContext):
    ...
    if p['pack_qty'] > 1:
        await cb.message.edit_text(f"📦 Введите количество <b>упаковок</b> для «{p['name']}» (в упаковке {p['pack_qty']} шт, в наличии: {p['stock']} упаковок):", parse_mode="HTML")
    else:
...
async def buy_qty_done(msg: Message, state: FSMContext):
    ...
    if p['pack_qty'] > 1:
        await msg.answer(f"✅ «{p['name']}» x{qty} упаковок (={total_qty} шт) = {p['price']*qty} {curr} добавлено!")
    else:
...
async def view_cart(cb: CallbackQuery):
    ...
    for i in items:
        ...
        if i['pack_qty'] > 1:
            text += f"• {i['name']} (x{i['pack_qty']}) x{i['quantity']} уп. = {i['price']*i['quantity']} {curr} ({total_qty} шт)\n"
        else:
...
async def edit_cart_start(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(buy_pid=pid)
    if p['pack_qty'] > 1:
        await cb.message.edit_text(f"📦 Новое количество упаковок для «{p['name']}» (в упаковке {p['pack_qty']} шт, в наличии: {p['stock']} упаковок, 0 = удалить):")
    else:
        await cb.message.edit_text(f"📦 Новое количество для «{p['name']}» (в наличии: {p['stock']} шт, 0 = удалить):")
    await state.set_state(SellerStates.cart_input_qty)
...
async def seller_stats(cb: CallbackQuery):
    ...
    text = f"📊 <b>Статистика магазина «{shop['shop_name']}»</b>\n\n"
    text += f"✅ Выполнено заказов: <b>{stats['total_orders']}</b>\n"
    text += f"💰 Заработано: <b>{stats['total_earned']}</b>\n"
    text += f"⏳ Активных заказов: <b>{stats['pending']}</b>"
    await cb.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def edit_product_start(cb: CallbackQuery):
    ...
    for p in prods:
        ...
        if p['pack_qty'] > 1:
            ...
            text = f"✏️ {p['name']} — {p['price']} {curr} ({p['stock']} шт)"
...
async def edit_product_menu(cb: CallbackQuery):
    ...
    curr = plural(p['currency'], p['price'])
    if p['pack_qty'] > 1:
        text = f"<b>{p['name']}</b>\n📦 Упаковка: {p['pack_qty']} шт\n💰 Цена за упаковку: {p['price']} {curr}\n📦 На складе: {p['stock']} упаковок"
    else:
...
async def edit_stock_start(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(edit_pid=pid)
    if p['pack_qty'] > 1:
        await cb.message.edit_text("📦 Введите новое количество упаковок (число):")
    else:
...
async def del_product_start(cb: CallbackQuery):
    ...
    for p in prods:
        ...
        if p['pack_qty'] > 1:
            ...
            text = f"🗑 {p['name']} — {p['price']} {curr} ({p['stock']} шт)"
...
async def product_pack_qty(msg: Message, state: FSMContext):
    ...
    if pack_qty > 1:
        await msg.answer(f"💰 Введите цену за <b>упаковку из {pack_qty} шт</b> (число):", parse_mode="HTML")
    else:
...
async def product_stock(msg: Message, state: FSMContext):
    ...
    await state.clear()
    if pack_qty > 1:
        await msg.answer(f"✅ «{data['prod_name']}» (x{pack_qty}) за {data['prod_price']} {curr} ({stock} упаковок) добавлен!")
    else:
